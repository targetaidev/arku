...
from arku.jobs import Job, JobStatus
from arku.worker import (
    FailedJobs,
    JobExecutionFailed,
    Retry,
    RetryJob,
...
def test_no_jobs(arku_redis: ArkuRedis, loop):
    class Settings:
    ...
    loop.run_until_complete(arku_redis.enqueue_job('foobar'))
...
async def test_health_check_pass(arku_redis):
    await arku_redis.set(default_queue_name + health_check_key_suffix, b'1')
    assert 0 == await async_check_health(None)


async def test_set_health_check_key(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('foobar', _job_id='testing')
    worker: Worker = worker(functions=[func(foobar, keep_result=0)], health_check_key='arku:test:health-check')
    await worker.main()
    with arku_redis.encoder_context(decode_responses=True):
        assert sorted(await arku_redis.keys('*')) == ['arku:test:health-check']
...
async def test_job_successful(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foobar', _job_id='testing')
...
async def test_job_retry(arku_redis: ArkuRedis, worker, caplog):
    async def retry(ctx):
        if ctx['job_try'] <= 2:
            raise Retry(defer=0.01)

    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('retry', _job_id='testing')
...
async def test_job_retry_dont_retry(arku_redis: ArkuRedis, worker, caplog):
    async def retry(ctx):
        raise Retry(defer=0.01)
...
async def test_job_retry_max_jobs(arku_redis: ArkuRedis, worker, caplog):
    async def retry(ctx):
        raise Retry(defer=0.01)
...
async def test_job_job_not_found(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('missing', _job_id='testing')
    worker: Worker = worker(functions=[foobar])
    await worker.main()
    assert worker.jobs_complete == 0
...
async def test_job_job_not_found_run_check(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('missing', _job_id='testing')
    worker: Worker = worker(functions=[foobar])
    with pytest.raises(FailedJobs) as exc_info:
...
async def test_retry_lots(arku_redis: ArkuRedis, worker, caplog):
    async def retry(ctx):
        raise Retry()
...
async def test_retry_lots_without_keep_result(arku_redis: ArkuRedis, worker):
    async def retry(ctx):
        raise Retry()
...
async def test_retry_lots_check(arku_redis: ArkuRedis, worker, caplog):
    async def retry(ctx):
        raise Retry()
...
async def test_cancel_error(arku_redis: ArkuRedis, worker, caplog):
    async def retry(ctx):
        if ctx['job_try'] == 1:
...
async def test_retry_job_error(arku_redis: ArkuRedis, worker, caplog):
    async def retry(ctx):
        if ctx['job_try'] == 1:
            raise RetryJob()
...
async def test_job_expired(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foobar', _job_id='testing')
    await arku_redis.delete(job_key_prefix + 'testing')
    worker: Worker = worker(functions=[foobar])
    assert worker.jobs_complete == 0
    assert worker.jobs_failed == 0
    assert worker.jobs_retried == 0
...
async def test_job_expired_run_check(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foobar', _job_id='testing')
    await arku_redis.delete(job_key_prefix + 'testing')
    worker: Worker = worker(functions=[foobar])
...
async def test_job_old(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foobar', _job_id='testing', _defer_by=-2)
    worker: Worker = worker(functions=[foobar])
    assert worker.jobs_complete == 0
    assert worker.jobs_failed == 0
    assert worker.jobs_retried == 0
...
async def test_str_function(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('asyncio.sleep', _job_id='testing')
...
async def test_startup_shutdown(arku_redis: ArkuRedis, worker):
    calls = []
...
async def test_exc_extra(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('error_function', _job_id='testing')
...
async def test_unpickleable(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    ...
    await arku_redis.enqueue_job('example', _job_id='testing')
...
async def test_log_health_check(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foobar', _job_id='testing')
...
async def test_remain_keys(arku_redis: ArkuRedis, worker):
    redis2 = Redis(host='localhost', port=6379, decode_responses=True)
    try:
        await arku_redis.enqueue_job('foobar', _job_id='testing')
        assert sorted(await redis2.keys('*')) == ['arku:job:testing', 'arku:queue']
        worker: Worker = worker(functions=[foobar])
        await worker.main()
        assert sorted(await redis2.keys('*')) == ['arku:queue:health-check', 'arku:result:testing']
        await worker.close()
        assert sorted(await redis2.keys('*')) == ['arku:result:testing']
    finally:
        await redis2.close()


async def test_remain_keys_no_results(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('foobar', _job_id='testing')
    with arku_redis.encoder_context(decode_responses=True):
        assert sorted(await arku_redis.keys('*')) == ['arku:job:testing', 'arku:queue']
    worker: Worker = worker(functions=[func(foobar, keep_result=0)])
    await worker.main()
    with arku_redis.encoder_context(decode_responses=True):
        assert sorted(await arku_redis.keys('*')) == ['arku:queue:health-check']


async def test_remain_keys_keep_results_forever_in_function(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('foobar', _job_id='testing')
    with arku_redis.encoder_context(decode_responses=True):
        assert sorted(await arku_redis.keys('*')) == ['arku:job:testing', 'arku:queue']
    ...
    await worker.main()
    with arku_redis.encoder_context(decode_responses=True):
        assert sorted(await arku_redis.keys('*')) == ['arku:queue:health-check', 'arku:result:testing']
    ttl_result = await arku_redis.ttl('arku:result:testing')
    assert ttl_result == -1


async def test_remain_keys_keep_results_forever(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('foobar', _job_id='testing')
    with arku_redis.encoder_context(decode_responses=True):
        assert sorted(await arku_redis.keys('*')) == ['arku:job:testing', 'arku:queue']
    worker: Worker = worker(functions=[func(foobar)], keep_result_forever=True)
...
async def test_run_check_passes(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('foobar')
    await arku_redis.enqueue_job('foobar')
    worker: Worker = worker(functions=[func(foobar, name='foobar')])
    assert 2 == await worker.run_check()


async def test_run_check_error(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('fails')
...
async def test_run_check_error2(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('fails')
    await arku_redis.enqueue_job('fails')
    worker: Worker = worker(functions=[func(fails, name='fails')])
...
async def test_return_exception(arku_redis: ArkuRedis, worker):
    async def return_error(ctx):
        return TypeError('xxx')

    j = await arku_redis.enqueue_job('return_error')
...
async def test_error_success(arku_redis: ArkuRedis, worker):
    j = await arku_redis.enqueue_job('fails')
    worker: Worker = worker(functions=[func(fails, name='fails')])
    await worker.main()
...
async def test_many_jobs_expire(arku_redis: ArkuRedis, worker, caplog):
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foobar')
    await asyncio.gather(*[arku_redis.zadd(default_queue_name, {f'testing-{i}': 1}) for i in range(100)])
    worker: Worker = worker(functions=[foobar])
    assert worker.jobs_complete == 0
    assert worker.jobs_failed == 0
    assert worker.jobs_retried == 0
...
async def test_repeat_job_result(arku_redis: ArkuRedis, worker):
    j1 = await arku_redis.enqueue_job('foobar', _job_id='job_id')
    assert isinstance(j1, Job)
    assert await j1.status() == JobStatus.queued

    assert await arku_redis.enqueue_job('foobar', _job_id='job_id') is None
...
async def test_queue_read_limit_equals_max_jobs(arku_redis: ArkuRedis, worker):
    for _ in range(4):
        await arku_redis.enqueue_job('foobar')

    assert await arku_redis.zcard(default_queue_name) == 4
    ...
    assert await arku_redis.zcard(default_queue_name) == 2
    ...
    await worker._poll_iteration()
    await asyncio.sleep(0.1)
    assert await arku_redis.zcard(default_queue_name) == 0
...
async def test_custom_queue_read_limit(arku_redis: ArkuRedis, worker):
    for _ in range(4):
        await arku_redis.enqueue_job('foobar')
...
async def test_custom_serializers(arku_redis_msgpack: ArkuRedis, worker):
    j = await arku_redis_msgpack.enqueue_job('foobar', _job_id='job_id')
...
async def test_deserialization_error(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('foobar', UnpickleFails('hello'), _job_id='job_id')
    worker: Worker = worker(functions=[foobar])
    with pytest.raises(FailedJobs) as exc_info:
        await worker.run_check()
...
async def test_incompatible_serializers_1(arku_redis_msgpack: ArkuRedis, worker):
    await arku_redis_msgpack.enqueue_job('foobar', _job_id='job_id')
    worker: Worker = worker(functions=[foobar])
    await worker.main()
    assert worker.jobs_complete == 0
    assert worker.jobs_failed == 1
...
async def test_incompatible_serializers_2(arku_redis: ArkuRedis, worker):
    await arku_redis.enqueue_job('foobar', _job_id='job_id')
    worker: Worker = worker(
        functions=[foobar], job_serializer=msgpack.packb, job_deserializer=functools.partial(msgpack.unpackb, raw=False)
    )
    await worker.main()
...
async def test_max_jobs_completes(arku_redis: ArkuRedis, worker):
    v = 0
    ...
    await arku_redis.enqueue_job('raise_second_time')
    await arku_redis.enqueue_job('raise_second_time')
    await arku_redis.enqueue_job('raise_second_time')
...
async def test_max_bursts_sub_call(arku_redis: ArkuRedis, worker, caplog):
    async def foo(ctx, v):
        return v + 1
    ...
    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('bar', 10)
...
async def test_max_bursts_multiple(arku_redis: ArkuRedis, worker, caplog):
    async def foo(ctx, v):
        return v + 1

    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foo', 1)
    await arku_redis.enqueue_job('foo', 2)
...
async def test_max_bursts_dont_get(arku_redis: ArkuRedis, worker):
    async def foo(ctx, v):
        return v + 1
...
async def test_non_burst(arku_redis: ArkuRedis, worker, caplog, loop):
    async def foo(ctx, v):
        return v + 1

    caplog.set_level(logging.INFO)
    await arku_redis.enqueue_job('foo', 1, _job_id='testing')
    worker: Worker = worker(functions=[func(foo, name='foo')])
    worker.burst = False
...
async def test_multi_exec(arku_redis: ArkuRedis, worker, caplog):
    c = 0
    ...
    caplog.set_level(logging.DEBUG, logger='arku.worker')
    await arku_redis.enqueue_job('foo', 1, _job_id='testing')
    worker: Worker = worker(functions=[func(foo, name='foo')])
...
async def test_abort_job(arku_redis: ArkuRedis, worker, caplog, loop):
    ...
    caplog.set_level(logging.INFO)
    await arku_redis.zadd(abort_jobs_ss, {b'foobar': int(1e9)})
    job = await arku_redis.enqueue_job('longfunc', _job_id='testing')
...
async def test_abort_job_before(arku_redis: ArkuRedis, worker, caplog, loop):
    async def longfunc(ctx):
        await asyncio.sleep(3600)
...
async def test_not_abort_job(arku_redis: ArkuRedis, worker, caplog, loop):
    ...
    caplog.set_level(logging.INFO)
    job = await arku_redis.enqueue_job('shortfunc', _job_id='testing')
...
async def test_job_timeout(arku_redis: ArkuRedis, worker, caplog):
    async def longfunc(ctx):
        await asyncio.sleep(0.3)

    caplog.set_level(logging.ERROR)
    await arku_redis.enqueue_job('longfunc', _job_id='testing')
